...
class RectProfitController: SNBaseViewController {
    ...
    fileprivate let tableView:UITableView = UITableView().then{
        ...
        $0.register(RectProfitCell.self)
    ...
    override func loadData() {
        ...
        let url = httpUrl + "/main/profitLog"
        Alamofire.request(url, method: .post, parameters:nil, headers:["X-AUTH-TOKEN":XKeyChain.get(TOKEN)] ).responseJSON {[unowned self] (res) in
            ...
            if  jsonData["code"].intValue == 1000{
                ...
                CNLog(jsonObj)
                self.model  = jsonObj.arrayValue.compactMap({RectProfitModel(jsonData: $0)})
...
extension RectProfitController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:RectProfitCell = tableView.dequeueReusableCell(forIndexPath: indexPath)
...
extension RectProfitController:DZNEmptyDataSetSource,DZNEmptyDataSetDelegate{

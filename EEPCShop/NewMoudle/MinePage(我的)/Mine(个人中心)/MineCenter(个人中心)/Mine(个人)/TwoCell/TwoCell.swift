...
class TwoCell: SNBaseTableViewCell {
    ...
    var model:ShopMsgModel?{
        didSet{
            ...
            self.shopProfitView.receiveProfit.text = "￥" + cellModel.todayreceive
            self.shopProfitView.rectProfit.text = "￥" +  cellModel.rectProfit
            self.shopProfitView.rmdProfit.text = "￥" + cellModel.rmdProfit
            self.shopProfitView.balanceProfit.text =  "￥" + cellModel.balance
...
//        $0.backgroundColor = Color(0x005eb7)
    ...
    let shopfuncView = BeShopView().then{
//        $0.backgroundColor = Color(0x005eb7)
        $0.backgroundColor = ColorRGB(red: 81.0, green: 130.0, blue: 255.0)

    }
    
    let shopProfitView = ShopProfitView().then{
        $0.backgroundColor = Color(0xffffff)
    }
    
    let toolView = ShopToolView().then{
        $0.backgroundColor = Color(0xffffff)
    }
    ...
    override func setupView() {
        ...
        self.baseView.addSubview(self.shopProfitView)
        ...
        self.baseView.snp.makeConstraints { (make) in
            ...
            make.height.snEqualTo(1350 + 160)
        }
        ...
        shopProfitView.snp.makeConstraints { (make) in
            make.left.equalToSuperview()
            make.right.equalToSuperview()
        ...
        rmdUserBanner.snp.makeConstraints { (make) in
            ...
            make.top.equalTo(shopProfitView.snp.bottom)
        ...
        rmdToolView.snp.makeConstraints { (make) in
            make.left.right.equalToSuperview()
            make.top.equalTo(toolView.snp.bottom).snOffset(20)
            make.height.snEqualTo(220)
        }
        ...
        rmdImageView.snp.makeConstraints { (make) in
            ...
            make.top.equalTo(rmdToolView.snp.bottom).snOffset(10)
        }

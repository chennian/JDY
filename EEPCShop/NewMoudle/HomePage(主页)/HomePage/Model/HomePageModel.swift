...
enum homePageType{
    ...
    case merchantByGoodType(HomePageModel)
    case spaceType
    case emptyType
    case mapType
...
class HomePageModel: SNSwiftyJSONAble {
    ...
    required init?(jsonData: JSON) {
        ...
        self.dis = jsonData["dis"].stringValue

        self.goods = jsonData["goods"].arrayValue.map({ (json) -> GoodModel in
            let model = GoodModel.init(jsonData: json)
            return model!
        })
